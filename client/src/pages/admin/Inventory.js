...
export default function Inventory() {
  ...
  const updateStock = async (id) => {
    ...
    await axios.put(`${API}/inventory/${id}`, { stockQuantity: Number(val) },
              ...
              return (
                <tr key={p._id} style={{ borderTop: "1px solid #f3f4f6" }}>
                  ...
                  <td style={{ padding: "0.75rem 1rem" }}>
                    <span style={{ background: p.category === "skin" ? "#F5E8EA" : "#E1F5EE",
                      color: p.category === "skin" ? "#7B2D3C" : "#047857",
                      padding: "0.2rem 0.6rem", borderRadius: "999px", fontSize: "0.75rem" }}>
                      {p.category}
                    </span>
                  </td>
                  <td style={{ padding: "0.75rem 1rem" }}>₹{(p.price / 100).toFixed(0)}</td>
