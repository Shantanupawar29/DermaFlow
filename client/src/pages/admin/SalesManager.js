...
export default function SalesManager() {
  ...
  const fetch = async () => {
    ...
    try {
      const [sRes, subRes, pRes] = await Promise.allSettled([
        axios.get(`${API}/sales/admin-all`, tok()),
        axios.get(`${API}/sales/subscriptions/admin-all`, tok()),
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await axios.post(`${API}/sales`, form, tok());
  ...
  const toggle = async (sale) => {
    try {
      await axios.put(`${API}/sales/${sale._id}`, { isActive: !sale.isActive }, tok());
      fetch();
  ...
  const deleteSale = async (id) => {
    ...
    await axios.delete(`${API}/sales/${id}`, tok());
    fetch();
  };
  ...
  const sendWinback = async () => {
    ...
    try {
      const r = await axios.post(`${API}/sales/winback`, { days: winback.days }, tok());
