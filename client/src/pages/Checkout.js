...
const Checkout = () => {
  ...
  useEffect(() => {
    if (userGlowPoints > 0 && totalPrice > 0) {
      ...
      const maxPointsPossible = Math.floor(maxDiscountAmount * 10);
      ...
      if (pointsApplied && pointsToUse > roundedMaxPoints) {
        setPointsToUse(roundedMaxPoints);
        setPointsDiscount((roundedMaxPoints / 100) * 10);
      }
    }
  ...
  const applyGlowPoints = () => {
    ...
    if (userGlowPoints < 100) { setPointsMsg('You need at least 100 Glow Points to redeem'); return; }
    if (maxPointsToUse < 100) { setPointsMsg('Maximum discount cannot exceed 50% of order total'); return; }
    const discountAmount = (maxPointsToUse / 100) * 10;
  ...
  const createOrderAndRedirect = async (orderData) => {
    try {
      ...
      const mappedItems = cartItems.map(item => {
        ...
        if (!productId) throw new Error(`Product ${item.name} is missing an ID`);
        return { product: productId, quantity: item.quantity, name: item.name, price: item.price };
      });
      const payload = {
        items: mappedItems,
      ...
      const errorMessage = error.response?.data?.message || error.response?.data?.error || error.message || 'Order failed. Please try again.';
      alert(errorMessage);
      return false;
    } finally {
      setLoading(false);
        ...
        handler: async (response) => {
          try {
            ...
            if (verifyRes.data.success) {
              await createOrderAndRedirect({ paymentMethod: 'razorpay' });
            } else {
  ...
  const handleSubmit = async (e) => {
    ...
    await createOrderAndRedirect({ paymentMethod: 'cod' });
    setLoading(false);
  };
        ...
        <div className="lg:col-span-2 space-y-4">
          <form onSubmit={handleSubmit}>
                    ...
                    <div className="ml-4 space-y-2">
                      {savedAddresses.map(addr => (
                        <label key={addr._id} className="flex items-start gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                          <input type="radio" name="savedAddress" checked={selectedAddressId === addr._id} onChange={() => handleSelectAddress(addr._id)} className="mt-1 flex-shrink-0" />
                          <div className="min-w-0">
                            <p className="font-medium text-sm">{addr.name}</p>
                            <p className="text-xs text-gray-600 break-words">{addr.street}, {addr.city}, {addr.state} - {addr.zipCode}</p>
              ...
              {useNewAddress && (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input type="text" placeholder="Street Address" required className="border p-2 rounded w-full text-sm"
                      value={formData.address.street} onChange={(e) => setFormData({...formData, address: {...formData.address, street: e.target.value}})} />
                    <input type="text" placeholder="City" required className="border p-2 rounded w-full text-sm"
                      value={formData.address.city} onChange={(e) => setFormData({...formData, address: {...formData.address, city: e.target.value}})} />
                    <select required className="border p-2 rounded w-full text-sm" value={formData.address.state}
                    ...
                    <input type="text" placeholder="Pincode" required maxLength={6} className="border p-2 rounded w-full text-sm"
                    ...
                    <input type="tel" placeholder="Phone Number" required className="border p-2 rounded w-full text-sm sm:col-span-2"
            ...
            <div className="bg-white rounded-lg shadow p-4 mb-4">
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center p-3 border rounded cursor-pointer hover:bg-gray-50">
                  ...
                  <div>
                    <span className="font-medium text-sm">Credit/Debit Card / UPI</span>
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-maroon text-white py-3 rounded-lg font-semibold hover:bg-maroon/90 transition disabled:bg-gray-400 text-sm">
            ...
            <div className="mb-4">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 min-w-0 border border-gray-300 rounded-lg p-2 text-sm"
                />
                <button
                  type="button"
                  onClick={applyCoupon}
                  className="flex-shrink-0 bg-maroon text-white px-3 py-2 rounded-lg text-sm hover:bg-maroon/90"
                >
                  Apply
                </button>
              </div>
              ...
              <div className="mb-4 p-3 bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-sm font-semibold text-amber-800 flex items-center gap-1">
                    <Sparkles size={14} /> Glow Points
                  </p>
                  <span className="text-base font-bold text-amber-700">{userGlowPoints}</span>
                </div>
                <div className="mb-2">
                  ...
                  <div className="w-full bg-amber-200 rounded-full h-1.5">
                    <div className="bg-amber-600 h-1.5 rounded-full transition-all duration-300"
                      style={{ width: `${Math.min((maxPointsToUse / userGlowPoints) * 100, 100)}%` }} />
                  </div>
                  <p className="text-xs text-amber-600 mt-1">
                    {pointsApplied ? `✅ Saving ₹${pointsDiscount.toFixed(2)}` : `Apply to save ₹${(maxPointsToUse / 100 * 10).toFixed(2)}`}
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  className={`w-full text-sm py-2 rounded-lg font-semibold transition ${
                    pointsApplied
                ...
                >
                  {pointsApplied ? `✓ ${pointsToUse} pts applied — Remove` : `✨ Apply ${maxPointsToUse} Points`}
                </button>
            ...
            <div className="border-t pt-3 space-y-2 text-sm">
              ...
              {pointsDiscount > 0 && (
                <div className="flex justify-between text-amber-600">
                  <span>Glow Points ({pointsToUse} pts)</span><span>-₹{pointsDiscount.toFixed(2)}</span>
                </div>
              )}
              ...
              <div className="flex justify-between font-bold text-base pt-2 border-t">
                <span>Total</span>
                <span className="text-maroon">₹{grandTotal.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
