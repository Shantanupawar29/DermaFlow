...
export default function Subscriptions() {
  ...
  const fetch = async () => {
    ...
    try {
      const [sRes, fRes, pRes] = await Promise.allSettled([
        axios.get(`${API}/subscriptions/mine`, tok()),
        axios.get(`${API}/subscriptions/freebies`, tok()),
  ...
  const subscribe = async () => {
    ...
    setSaving(true);
    try {
      await axios.post(`${API}/subscriptions`, newSub, tok());
      setShowAdd(false); setNewSub({ productId:'', plan:'monthly' }); fetch();
    } catch(e) { alert(e.response?.data?.message || 'Failed to subscribe'); }
    setSaving(false);
  };

  const togglePause = async (id) => {
    try { await axios.put(`${API}/subscriptions/${id}/pause`, {}, tok()); fetch(); }
    catch(e) { alert('Failed'); }
  };

  const cancel = async () => {
    try {
      await axios.put(`${API}/subscriptions/${cancelModal}/cancel`, { reason: cancelReason }, tok());
